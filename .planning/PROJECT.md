...
| D7 | locked | Together AI is the sentiment provider, hosting `cardiffnlp/twitter-roberta-base-sentiment-latest`; new env var `TOGETHER_API_KEY` | Together's fast batch endpoint meets the 5s budget natively |
| F1 | locked | `.env.example`: add `TOGETHER_API_KEY` + `SENTIMENT_DAILY_CHAR_BUDGET` (default ~200000); **drop** `HUGGINGFACE_API_TOKEN` (never to be introduced) | Aligns env contract with D7 + D1 |
| F2 | locked | `lib/api/sentiment.ts` ships `preprocessText()` replacing `@username` → literal `@user` and any `http*` URL → literal `http` before submitting to the model; required by the cardiffnlp model card and unit-tested per D6 | Skipping preprocessing is a silent quality regression |
| F3 | locked | `app/api/cron/refresh/route.ts` exports `const maxDuration = 30` | 3-tier DAG hits ~12s under load; Vercel default 10s would time out |
